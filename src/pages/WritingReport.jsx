...
export default function WritingReport() {
  ...
  // ✅ timeUsedSec(구버전) / timeUsedByTaskSec(신버전) 모두 커버
  const totalUsedSec = (() => {
    ...
    <Page>
      <Wrap>
        ...
        <BottomRow>
          <BtnGhost
            type="button"
            onClick={() => nav(`/writing/mock/${encodeURIComponent(mockId)}`)}
          >
            다시 풀기
          </BtnGhost>
          <BtnPrimary type="button" onClick={() => nav("/writing")}>
            목록으로
          </BtnPrimary>
        </BottomRow>
      </Wrap>
